...
bool InputDevice::InitDevice(LPDIRECTINPUT8 directInput, int type, HWND window)
{
	...
	result = this->device->SetCooperativeLevel(
		...
		DISCL_NONEXCLUSIVE | DISCL_FOREGROUND);		// Tham số thứ hai xác định độ ưu tiên của bàn phím và 
													// chuột đối với chương trình của chúng ta. Giá trị 
													// chung thường được truyền khi làm việc với bàn phím 
													// và chuột là DISCL_NONEXCLUSIVE và DISCL_FOREGROUND
